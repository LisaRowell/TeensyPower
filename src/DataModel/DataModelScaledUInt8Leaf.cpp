...
#include "DataModelScaledUInt8Leaf.h"
...
DataModelScaledUInt8Leaf::DataModelScaledUInt8Leaf(const char *name, DataModelNode *parent)
    : DataModelRetainedValueLeaf(name, parent) {
}

DataModelScaledUInt8Leaf & DataModelScaledUInt8Leaf::operator = (const ScaledUInt8 &value) {
...
void DataModelScaledUInt8Leaf::sendRetainedValue(DataModelSubscriber &subscriber) {
    if (hasValue()) {
        etl::string<maxStringLength> valueStr;
        value.toString(valueStr);
